...
public sealed class PromptChatClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            new("system", "你是 IoTCoWork 工作台里的简洁中文助手。用户闲聊或询问时正常回答；如果用户要作图，提醒他可以直接描述画面。"),
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            return await SendChatOnceAsync(settings, endpoint, messages, temperature, useLocalProxy: true, cancellationToken);
        }
        catch (PromptChatProxyUnavailableException)
        {
            return await SendChatOnceAsync(settings, endpoint, messages, temperature, useLocalProxy: false, cancellationToken);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new HttpRequestException($"会话接口返回 {(int)response.StatusCode} {response.ReasonPhrase}: {ExtractErrorMessage(raw)}");
        }
    ...
    {
        var root = string.IsNullOrWhiteSpace(baseUrl) ? StudioSettings.DefaultAiGatewayBaseUrl : baseUrl.Trim();
        if (!root.EndsWith('/'))
        {
            root += "/";
        }
    ...
    private static string ExtractErrorMessage(string raw)
    {
        ...
        try
        {
            ...
            {
                message = NormalizeChatErrorMessage(message);
    ...
    private static string NormalizeChatErrorMessage(string message)
    {
        const string imagePrefix = "无法连接图像接口";
        const string chatPrefix = "无法连接会话接口";

        return message.Equals(imagePrefix, StringComparison.Ordinal)
            ? chatPrefix
            : message.StartsWith(imagePrefix + "：", StringComparison.Ordinal)
                ? chatPrefix + message[imagePrefix.Length..]
                : message;
    }
