...
public sealed class ImageGenerationClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new HttpRequestException($"图像接口返回 {(int)response.StatusCode} {response.ReasonPhrase}: {ExtractErrorMessage(raw)}");
        }
    ...
    private static Uri BuildImageEndpoint(string baseUrl, StudioImageRequest request)
    {
    ...
    private static bool ApplyKnownAdvancedField(ImageGenerationRequest request, string key, JsonNode value)
    {
        switch (key.Trim())
        {
            ...
            case "n":
                ...
                return true;
            case "size":
                request.Size = StringFromJson(value);
                return true;
            case "quality":
                request.Quality = StringFromJson(value);
                return true;
            case "style":
                request.Style = StringFromJson(value);
                return true;
            case "background":
                request.Background = StringFromJson(value);
                return true;
            case "output_format":
                request.OutputFormat = StringFromJson(value);
                return true;
            case "output_compression":
                request.OutputCompression = ClampIntFromJson(value, request.OutputCompression ?? 100, 0, 100);
                return true;
            case "moderation":
                request.Moderation = StringFromJson(value);
                return true;
            case "input_fidelity":
